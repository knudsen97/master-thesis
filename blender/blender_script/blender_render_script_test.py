...
try:
    import gt_face_change_script_test
    importlib.reload(gt_face_change_script_test)
except ImportError:
...
# TODO: find the spacing for the biggest box

def delete_all_boxes():
    """detete all boxes except the original box
    TODO: delete all boxes in a collection
    """
...
def add_box_at_random(number_of_boxes, _boxes):
    ...
    TODO: add them at random location one at a time for every 3 frame until frame 20 or something 
...
def face_change_handler(FaceColor: gt_face_change_script_test.FaceColor):
    def handler(scene):
...
def main():
    ...
    face_changer = gt_face_change_script_test.FaceColor(added_boxes)
