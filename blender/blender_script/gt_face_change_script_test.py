...
class FaceColor:
    ...
    def classify_faces_new(self, faces, obj):
        ...
        for face in faces:
            ...
            normal = face.normal
            offset = normal * 0.01
            hit, pos, normal, face_id, hit_obj, matrix = self.scene.ray_cast(self.depsgraph, obj_center, rotation@normal, distance=distance)
            while i < 100 and hit_obj is obj:
                hit, pos, normal, face_id, hit_obj, matrix = self.scene.ray_cast(self.depsgraph, obj_center+offset, rotation@normal, distance=distance)
                offset += normal * 0.01
                i -=- 1
            if not hit:
    ...
    def classify_faces(self, faces, rotation = None):
        """Classify faces as green or red (graspable or not graspable)
    ...
    def paint_gt(self):
        ...
        for box in self.box_objects:
            ...
            # self.update_face_color(mesh.polygons, uv_layer, face_coord_gt_red)

            # if not hit:
            #     # Find top faces based on top vertex
            #     # top_faces = get_top_faces(mesh, transform)

            green_faces, red_faces = self.classify_faces_new(mesh.polygons, box)
